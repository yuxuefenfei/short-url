...
export const useUrlStore = defineStore("url", () => {
  ...
  const addRecentUrl = (urlData) => {
    // 移除已存在的相同URL
    const filteredUrls = recentUrls.value.filter(
      (url) => url.shortKey !== urlData.shortKey,
    );
  ...
  const addMultipleUrls = (urls) => {
    ...
    urls.forEach((urlData) => {
      // 移除重复项
      updatedUrls = updatedUrls.filter(
        (url) => url.shortKey !== urlData.shortKey,
      );
      // 添加到开头
  ...
  const searchRecentUrls = (query) => {
    ...
    return recentUrls.value.filter(
      (url) =>
